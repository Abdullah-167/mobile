...
const MobileData = () => {
      ...
      if (Object.values(newFilters).every((value) => !value)) {
        ...
        setTimeout(() => {
          const filteredResults = mobileData.filter((mobile) => {
            ...
            if (newFilters.make)
              makeFilter =
                mobile.make.toLowerCase() === newFilters.make.toLowerCase();
  
            let displaySizeFilter = true;
            if (newFilters.displaySize)
              displaySizeFilter =
                mobile.displaySize >= Number(newFilters.displaySize);
  
            let ramFilter = true;
            if (newFilters.ram) ramFilter = mobile.ram >= Number(newFilters.ram);
  
            let batteryFilter = true;
            if (newFilters.battery)
              batteryFilter = mobile.battery >= Number(newFilters.battery);
  
            let osFilter = true;
            if (newFilters.os)
              osFilter = mobile.os.toLowerCase() === newFilters.os.toLowerCase();
  
            let cameraFilter = true;
            if (newFilters.camera)
              cameraFilter = mobile.camera >= Number(newFilters.camera);
    ...
    // Define the type for the filter parameter
    type FilterKey = keyof Filters;
  
    const handleFilterRemove = (filter: FilterKey) => {
      const newFilters = { ...filters };
      delete newFilters[filter]; // Remove the selected filter
      setFilters(newFilters);
  
      // Reapply the remaining filters after removal
      handleFilter(newFilters); // Ensure the filters are recalculated
    };
